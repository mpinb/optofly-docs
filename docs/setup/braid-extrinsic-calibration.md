...
trigger, visual stimuli) is behaving correctly. Once every calibration
step is done, see [Full-pipeline dry run with a
laser](../workflow.md#full-pipeline-dry-run-with-a-laser) in the Workflow
page for a way to test that too, using this same laser trick.
